...
def main():
  ...
  for opt in range(num_iterations):
    ...
    # rec += (time.time() - t1) * 1000
    print(f'Time in epoch {opt} is {np.round(time.time() - tt,3)}, total time: {np.round(time.time() - tt0,3)}')
  # print(f'Time for {num_iterations} iterations of pytorch CPU is {rec}')
